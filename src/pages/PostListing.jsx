...
export default function PostListing() {
  ...
  async function handleSubmit() {
    ...
    // Get farmer_id from logged in user first
    let farmer_id = user?.farmer_id

    // If not in localStorage, look up by phone number (most reliable)
    if (!farmer_id) {
      const { data: farmerByPhone } = await supabase
        .from('farmers')
        .select('id')
        .eq('phone', user?.phone)
        .single()
      farmer_id = farmerByPhone?.id
    }

    // Last resort — look up by name
    if (!farmer_id) {
      const { data: farmerByName } = await supabase
        .from('farmers')
        .select('id')
        .eq('name', user?.full_name)
        .single()
      farmer_id = farmerByName?.id
    }

    if (!farmer_id) {
      setError('Could not find your farmer profile. Please contact support.')
      setLoading(false)
      return
    }
  ...
  return (
    <div style={{ background: '#F4F6F4', minHeight: '100vh', paddingBottom: '80px' }}>
      ...
      <div style={{
        ...
        <h1 style={{
          ...
          color: '#FFFFFF', marginBottom: '4px', letterSpacing: '-0.3px'
        }}>
          Post your produce
        </h1>
        ...
        <div style={{
          ...
          {step === 1 && (
            <div style={{ padding: '24px 20px' }}>
              <div style={{
                fontSize: '14px', fontWeight: '800', color: '#1B6B45',
                marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px'
              }}>
                <span style={{
                  width: '24px', height: '24px', background: '#E8F7EE',
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: '14px', background: '#1B6B45',
          ...
          {step === 2 && (
            <div style={{ padding: '24px 20px' }}>
              ...
              {form.quantity_kg && form.price_per_kg && (
                <div style={{
                  background: '#E8F7EE', border: '1px solid #BBE8D0',
                  borderRadius: '14px', padding: '14px 16px', marginBottom: '20px',
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between'
                }}>
                  <span style={{ fontSize: '13px', color: '#2A9660', fontWeight: '600' }}>
                    Estimated total value
                  </span>
                  <span style={{ fontSize: '18px', fontWeight: '800', color: '#1B6B45' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: '#1B6B45', border: 'none', cursor: 'pointer',
                    fontSize: '15px', fontWeight: '800', color: '#FFFFFF'
                  }}>
                  Review listing →
                </button>
          ...
          {step === 3 && (
            <div style={{ padding: '24px 20px' }}>
              ...
              {/* Preview card */}
              <div style={{
                background: '#0F1A14', borderRadius: '20px', overflow: 'hidden',
                marginBottom: '20px', border: '1px solid rgba(201,168,76,0.15)'
              }}>
                <div style={{ padding: '16px', display: 'flex', gap: '14px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '20px', fontWeight: '800', color: '#C9A84C' }}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  background: '#1B6B45', display: 'flex', alignItems: 'center',
                ...
                <div>
                  <div style={{ fontSize: '13px', fontWeight: '700', color: '#1B6B45' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    border: 'none', cursor: 'pointer',
                    fontSize: '15px', fontWeight: '800', color: '#0F1A14',
